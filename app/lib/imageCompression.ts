...
export function validateImageFile(file: File): { valid: boolean; error?: string } {
  if (!file.type.startsWith('image/')) {
    return {
      valid: false,
      error: `"${file.name}" is not an image file. Please upload JPG, PNG, or WEBP images only.`,
    };
  }
  ...
  if (!allowedTypes.includes(file.type)) {
    return {
      valid: false,
      error: `"${file.name}" has unsupported format. Supported: JPG, PNG, WEBP, GIF.`,
    };
  }
  ...
  if (file.size > 20 * 1024 * 1024) {
    return {
      valid: false,
      error: `"${file.name}" is too large (${(file.size / 1024 / 1024).toFixed(1)}MB). Maximum file size is 20MB.`,
    };
  }

...
async function updateDonationForCenters(
  centerId: number | string,
  donationAmount: number
) {
  const [center]: any = await db.query(
    "SELECT recieved FROM centers WHERE id = ?",
    [centerId]
  );
  if (!center?.length) {
  ...
  try {
    ...
    await db.query("UPDATE centers SET recieved = ? WHERE id = ?", [
      newReceived,
      centerId,
    ]);
  }
...
export async function recordVerifiedDonation(
  ...
  const [existing]: any = await db.query(
    "SELECT id FROM transactions WHERE refrence = ? LIMIT 1",
  ...
  const campaignId =
    input.campaign_id ??
    (meta.campaign_id ? Number(meta.campaign_id) : null) ??
    null;
  const centerIdInput =
    input.center_id ??
    (meta.center_id != null ? String(meta.center_id) : null);
  ...
  const buildDonors = (name: string) => {
    ...
    const existing =
      typeof cam.donors === "string" ? JSON.parse(cam.donors) : cam.donors;

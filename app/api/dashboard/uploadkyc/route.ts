  ...
  ON DUPLICATE KEY UPDATE
    ...
    created_at = NOW()`,
      [
        ...
        body.formData.city,
        body.formData.state,
        body.formData.documentType,
        body.formData.documentNumber,
        body.documentUrl,
        Math.floor(Math.random() * 7888888722),
      ],
    );
    await db.query('UPDATE users SET is_Verified = ? WHERE id = ?', [1, userid]);

    return NextResponse.json({ message: 'Successfully uploaded kyc' }, { status: 200 });
  } catch (_error) {
    console.log(_error);
    return NextResponse.json({ error: 'Error uploading KYC' }, { status: 500 });
  }
}

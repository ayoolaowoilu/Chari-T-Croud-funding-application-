...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const requiredFields = [
      ...
      'address', 'website', 'about', 'geo_location', 'bank_details'
    ]
    ...
    const requiredBankFields = [
      ...
      'bank_code', 'branch', 'swift_code', 'subAccountCode'
    ]
    ...
    const [centerResult]: any = await db.query(
      `INSERT INTO centers (
        ...
        geo_location, bank_details , verification_documents
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? , ?)`,
      [
        body.name,
        ...
        body.logourl,
        body.geo_location,
        JSON.stringify(body.bank_details),body.verification_documents
      ]
    )

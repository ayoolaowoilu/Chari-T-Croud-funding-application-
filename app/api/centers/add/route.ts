  ...
  try {
    ...
    for (const field of requiredFields) {
      if (!body[field as keyof CenterRegistrationPayload]) {
        ...
        return NextResponse.json({ error: `${field} is required` }, { status: 200 });
      }
    }
    ...
    for (const field of requiredBankFields) {
      if (!bank[field as keyof typeof bank]) {
        console.log(field, 'required');
        return NextResponse.json({ error: `bank_details.${field} is required` }, { status: 200 });
      }
    }
    ...
    if (!body.logourl) {
      ...
      return NextResponse.json({ error: 'Logo is required' }, { status: 200 });
    }

...
export async function POST(request: NextRequest) {
  try {
    ...
    const updateDonationForCenters = async () => {
      const [center]: any = await db.query(
        "SELECT recieved , total_donators FROM centers WHERE id = ?",
        [body.center_id]
      );
      ...
      if (body.center_id) {
        await db.query(
          "UPDATE centers SET recieved = ? , total_donators = ? WHERE id = ?",
          [newrecieved, oldDonators + 1, body.center_id]
        );
      }
    };
    ...
    const [owner_details]: any = await db.query(
      body.center_id
        ? "SELECT id, recieved FROM centers WHERE id = ?"
        : "SELECT id, recieved FROM users WHERE id = ?",
    ...
    const buildDonors = (name: string) => {
      ...
      const existing =
        typeof campaign[0].donors === "string"
      ...
      if (donated_rate > 50) {
        await db.query(
          ...
          [
            ...
            "verified_safe" as Campaign["safety_rating"],
            body.campaign_id,
          ]
        );
    ...
    if (!body.isAuthed && body.isBlind) {
      ...
      deleteRedisData(`campaign:${body.campaign_id}:type1`);
      deleteRedisData(`campaign:${body.campaign_id}:type2`);
    ...
    if (body.isAuthed && body.isBlind) {
      ...
      const payer_donations =
        Number(payer_details[0].donations) + Number(body.amount);

...
function getSecret(): string {
  return process.env.PAYSTACK_SECRET_KEY || process.env.NEXT_PUBLIC_PAYSTACK_SECRET_KEY || '';
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const campaign_id = Number(meta.campaign_id || field('campaign_id') || 0) || null;
    const center_id = meta.center_id || field('center_id') || null;
    const platform_fee = Number(meta.platform_fee || field('platform_fee') || 0);
    const isBlind =
      meta.isBlind === true || meta.isBlind === 'true' || field('is_blind') === 'true';
    ...
    console.error('Paystack webhook _error:', _error);

...
const CATEGORIES = [
  {
    ...
    label: 'Education',
    icon: BookOpen,
    description: 'Schools, scholarships & training programs',
  ...
  },
  {
    ...
    label: 'Community',
    icon: Globe,
    description: 'Local development & community support',
  ...
  },
  {
    ...
    label: 'Business',
    icon: Banknote,
    description: 'Startups & economic growth initiatives',
...
export default function StartCausePage() {
      ...
      try {
        ...
        if (file.size > MAX_FILE_SIZE) {
          ...
          compressionResult = await compressImageIfNeeded(file, (progress) => {
            ...
            setImages((prev) =>
              prev.map((img) =>
                img.publicId === uploadId ? { ...img, uploadProgress: 5 + progress * 0.35 } : img,
              ),
            );
          });
        ...
        setImages((prev) =>
          prev.map((img) => (img.publicId === uploadId ? { ...img, uploadProgress: 45 } : img)),
        );
        ...
        if (_error.message === 'Upload cancelled') {
  ...
  const deleteDraft = async (id: number) => {
    try {
      ...
      console.error('Failed to delete draft:', _err);
    }
  };
    ...
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-10">
        ...
        <p className="text-slate-500 ml-13">
          Continue where you left off or start a fresh campaign
        </p>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="text-center py-20 px-8">
            ...
            <p className="text-slate-500 mb-8 max-w-sm mx-auto">
              Start creating your first fundraising cause and make a real impact in your community.
            </p>
            <button
              onClick={() => setStage(2)}
  ...
  const renderStage2 = () => (
    <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 space-y-8">
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between">
            <p className="text-xs text-slate-500">
              Be specific and emotional — stories drive donations
            </p>
        ...
        {/* Location */}
        <div className="space-y-2">
          <label className="flex items-center gap-2 text-sm font-semibold text-slate-900"></label>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div
                      className={`p-2 rounded-lg transition-colors ${isSelected ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-500 group-hover:bg-slate-200'}`}
                    >
                      <Icon className="w-5 h-5" />
                    </div>
                    <div>
                      <p
                        className={`font-semibold text-sm ${isSelected ? 'text-slate-900' : 'text-slate-700'}`}
                      >
                        {cat.label}
                      </p>
    ...
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 mb-6 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 leading-relaxed">
            Images larger than <strong className="text-slate-700">1MB</strong> are automatically
            compressed before upload. Your image quality is preserved while ensuring fast, reliable
            uploads.
          </p>
        </div>
      </div>
      ...
      {compressionStats && compressionStats.compressed > 0 && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <p className="text-sm text-emerald-800">
            <strong>{compressionStats.compressed}</strong> image
            {compressionStats.compressed !== 1 ? 's' : ''} compressed, saving{' '}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
              ...
              {img.isUploading ? (
                <div className="w-full h-full flex flex-col items-center justify-center bg-slate-50 p-4">
                  {img.compressionStatus === 'compressing' ? (
                    <>
                      ...
                      <span className="text-[10px] text-slate-400 mt-1 text-center">
                        {img.originalSize ? formatFileSize(img.originalSize) : 'Large file'} →
                        optimized
                      </span>
                    </>
                  ...
                  <span className="text-[10px] text-slate-400 mt-2 font-mono">
                    {Math.round(Math.min(img.uploadProgress, 100))}%
                  </span>
                </div>
              ) : (
                <>
          ...
          >
            {isUploading ? (
              ...
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center mb-2 transition-colors ${dragActive ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-slate-200'}`}
              >
        ...
        {images.length === 0 && !isUploading && (
          ...
          >
            <div
              className={`w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center transition-colors ${dragActive ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-300'}`}
            >
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
              ...
              <p className="text-2xl font-bold text-slate-900">₦{goal?.toLocaleString() || '0'}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
              <div className="flex items-center gap-2 mb-2">
            ...
            <div>
              ...
              <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
                {images.map((img, idx) => (
                  <div
                    key={idx}
                    className={`aspect-square rounded-lg overflow-hidden bg-slate-100 border-2 ${
      ...
      <div className="w-full max-w-lg mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm p-10 text-center">
          ...
          <p className="text-slate-500 mb-10 max-w-sm mx-auto">
            <span className="font-semibold text-slate-900">"{causeName}"</span> is now live and
  ...
  const renderStage6 = () => (
    <div className="w-full max-w-lg mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="bg-white rounded-2xl border border-red-200 shadow-sm p-10 text-center">
        ...
        {errorMsg && (
          <p className="text-slate-500 text-sm mb-8 bg-slate-50 rounded-lg p-3 border border-slate-100">
            {errorMsg}
          </p>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={() => {
              ...
              setHasError(false);
              setStage(
                errorType === 'UPLOAD_IMG' || errorType === 'M_IMG' || errorType === 'IMGS' ? 3 : 4,
              );
            }}
              ...
              return (
                <div key={s.id} className="flex items-center flex-1 last:flex-none">
                  <div className="flex flex-col items-center">
                    ...
                    <span
                      className={`text-[10px] font-semibold mt-1.5 hidden sm:block ${isActive ? 'text-slate-900' : 'text-slate-400'}`}
                    >
                      {s.label}
                    </span>
                  </div>

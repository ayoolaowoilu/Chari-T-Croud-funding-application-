...
export default function Page() {
  ...
  useEffect(() => {
    if (!id) return;
    fetchOneCauseById(Number(id), 1).then(setCampaign).catch(console.error).finally(() => setLoading(false));
  }, [id]);
  ...
  if (!isCenter && campaign.goal === campaign.raised && campaign.raised > 0) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <NavBar />
      <div className="text-center">
  ...
  const shareUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/causes/cause?id=${id}&name=${campaign.name}`;

...
import { Heart, Mail, MessageSquare, Shield, UserCircle, XCircle } from 'lucide-react';
...
export default function Page() {
  ...
  const presetAmounts = useMemo(() => {
    if (currency === 'NG') {
      return ['1000', '5000', '10000', '250000', '500000'];
    } else {
  ...
  if (!campaign) {
    ...
    return (
      <>
        <div className="min-h-screen bg-[var(--background)] text-slate-900 flex items-center justify-center p-4 md:p-8">
          <NavBar />
          ...
          <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8">
                  <div className="backdrop-blur-md bg-white/80 border border-gray-200 rounded-2xl p-6 shadow-lg">
                    ...
                    <p className="text-gray-600 text-sm leading-relaxed">
                      Your contribution powers innovation and helps build a better future for
                      communities worldwide.
                    </p>
                    <div className="flex items-center gap-2 mt-4 text-[var(--brand)] text-sm font-medium">
            ...
            <div className="order-1 lg:order-2 w-full max-w-md mx-auto lg:mx-0 mt-5">
              <div className="mb-8">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[var(--brand-soft)] border border-teal-100 text-[var(--brand)] text-xs font-medium mb-4">
                  <Heart className="w-3 h-3 fill-current" />
                  <span>{campaign.category}</span>
                </div>

                <h1 className="text-4xl md:text-5xl font-light tracking-tight mb-3 text-gray-900">
                  {name} <span className="text-[var(--brand)] font-normal">Mission</span>
                </h1>
                {campaign.center_id && (
                  <div className="font-semibold text-gray-900 text-sm inline-flex items-center">
                ...
                <p className="text-gray-600 text-sm leading-relaxed">
                  Join thousands of supporters making a difference. Every contribution brings us
                  closer to our goal.
                </p>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                    ...
                    {presetAmounts.map((amt) => (
                      ...
                      ${
                        amount === amt && customAmount === ''
                          ? 'bg-[var(--brand)] border-[var(--brand)] text-white shadow-lg shadow-teal-200/40'
                          : 'bg-white border-gray-200 text-gray-700 hover:border-blue-300 hover:bg-[var(--brand-soft)]'
                      }
                    `}
                      >
                        {currency == 'NG' ? '₦' : currency == 'USD' ? '$' : '£'}
                        {amt}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="relative">
                    ...
                    ${
                      amount === 'custom'
                        ? 'border-[var(--brand)] ring-2 ring-teal-100'
...

                {
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <small className="text-gray-500 text-xs font-medium uppercase tracking-wider">
                        Optional tip (supports Chari-T)
                      </small>
                      {platFormFee > 0 && (
                        <span className="text-xs text-emerald-600 font-medium">
                          Campaign gets {currency == 'NG' ? '₦' : currency == 'USD' ? '$' : '£'}
        ...
        `}
                ...
                <PaystackPopup
                  ...
                  platform_fee={platFormFee}
                  name={
                    isBlind ? null : status == 'authenticated' ? session.user.name : formData.name
                  }
                  isBlind={isBlind}

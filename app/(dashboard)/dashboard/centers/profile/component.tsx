...
interface Props {
    centerId: number
    initialCenter: CenterProfile | null
    initialError: boolean
}

export default function CenterProfileClient({ centerId, initialCenter, initialError }: Props) {
    const [center, setCenter] = useState<CenterProfile | null>(initialCenter)
    const [loading, setLoading] = useState(!initialCenter && !initialError)
    ...
    // Only fetch on client if we didn't get initial data
    useEffect(() => {
        if (!initialCenter && !initialError) {
            fetchCenter()
        } else if (initialCenter) {
            // Set up welcome popup for SSR data
            if (initialCenter.campaigns && initialCenter.campaigns.length > 0) {
                const latest = initialCenter.campaigns[0]
                setLatestCampaign(latest)
                const hasSeenWelcome = sessionStorage.getItem(`welcome-${centerId}`)
                if (!hasSeenWelcome) {
                    setShowWelcome(true)
    ...
    }, [centerId, initialCenter, initialError])
        ...
        return (
            <div className="min-h-screen bg-[#f3f2ef]">
                ...
                <div className="mx-auto max-w-lg px-4 py-16 text-center sm:py-24">
                    ...
                    <button
                        onClick={fetchCenter}
                        className="mt-4 rounded-full bg-gray-900 px-5 py-2 text-sm font-medium text-white transition-all duration-200 hover:bg-gray-800 hover:shadow-lg active:scale-95 sm:mt-5 sm:px-6 sm:py-2.5"
                    >
                        Retry
                    </button>
                </div>
        ...
        <div className="relative min-h-screen bg-[#f3f2ef]">
            ...
            {showWelcome && latestCampaign && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-3 backdrop-blur-sm sm:p-4">
                    <div className="relative w-full max-w-sm animate-in fade-in zoom-in-95 overflow-hidden rounded-2xl bg-white shadow-2xl duration-200 sm:max-w-md">
                        <div className="relative h-40 bg-gray-100 sm:h-48">
                            {getImageUrl(latestCampaign.main_img) ? (
                                <img src={getImageUrl(latestCampaign.main_img)!} alt={latestCampaign.name} className="h-full w-full object-cover" />
                            ) : (
                            ...
                            <button
                                ...
                                className="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-black/40 text-white transition-colors hover:bg-black/60 sm:right-3 sm:top-3 sm:h-8 sm:w-8"
                            >
                        ...
                        <div className="p-4 sm:p-6">
                            <div className="mb-3 flex items-center gap-3 border-b border-gray-100 pb-3 sm:mb-4 sm:pb-4">
                                <div className="h-9 w-9 overflow-hidden rounded-full bg-gray-100 shadow-sm ring-2 ring-white sm:h-10 sm:w-10">
                                    {center.logourl ? (
                                        <img src={center.logourl} alt={center.name} className="h-full w-full object-cover" />
                                    ) : (
                                        <div className="flex h-full w-full items-center justify-center bg-gray-200">
                                            <span className="text-xs font-bold text-gray-500 sm:text-sm">{center.name.charAt(0).toUpperCase()}</span>
                                        </div>
                                    )}
                                </div>
                            ...
                            <h3 className="mb-1.5 text-lg font-bold text-gray-900 sm:mb-2 sm:text-xl">{latestCampaign.name}</h3>
                            <p className="mb-4 line-clamp-3 text-sm leading-relaxed text-gray-600 sm:mb-6">{latestCampaign.details}</p>
            ...
            {showDonateModal && (
                <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 backdrop-blur-sm sm:items-center sm:p-4">
                    <div className="relative w-full max-w-sm animate-in slide-in-from-bottom rounded-t-2xl bg-white p-5 shadow-2xl duration-200 sm:rounded-2xl sm:p-6">
                        <button
                            onClick={() => {
                            ...
                            className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full text-gray-500 transition-colors hover:bg-gray-100 sm:right-4 sm:top-4"
                        >
                        ...
                        <div className="mb-5 text-center sm:mb-6">
                            ...
                            <p className="mt-1 text-xs text-gray-500 sm:text-sm">Your contribution makes a difference</p>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="relative">
                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-base font-semibold text-gray-500 sm:text-lg">₦</span>
                                    <input
                                        type="number"
                            ...
                            <PaystackPopup
                                ...
                                onSuccess={() => {
                                    console.log("success popup opened")
                                }}
                                onCancel={() => {
            ...
            <div className="mx-auto max-w-5xl px-3 py-4 sm:px-4 sm:py-6">
                {/* Main Card */}
                <div className="overflow-hidden rounded-xl bg-white shadow-sm transition-shadow duration-300 hover:shadow-md sm:rounded-xl">
                    {/* Banner */}
                    <div className="relative h-36 overflow-hidden bg-gray-200 sm:h-48 md:h-56">
                        <img
                            src={randomBanner}
                            alt="Banner"
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between">
                            <div className="flex flex-col sm:flex-row sm:items-end sm:gap-5">
                                ...
                                <div className="mt-2 sm:mt-0 sm:mb-1">
                                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                                        <h1 className="text-lg font-semibold text-gray-900 sm:text-xl md:text-2xl">{center.name}</h1>
                            ...
                            <div className="mt-3 flex gap-2 sm:mt-0 sm:gap-3">
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-1.5 rounded-full border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-600 shadow-sm transition-all duration-200 hover:border-gray-400 hover:bg-gray-50 hover:shadow-md active:scale-95 sm:gap-2 sm:px-5"
                                >
                                    <svg className="h-3.5 w-3.5 sm:h-4 sm:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path
                                            strokeLinecap="round"
                                            strokeLinejoin="round"
                                            strokeWidth={2}
                                            d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-4.5 0V6a2.25 2.25 0 012.25-2.25h3.75a2.25 2.25 0 012.25 2.25v3.75m-9 0h9"
                                        />
                                    </svg>
                                    Website
                                </button>
                ...
                <div className="mt-3 grid gap-3 sm:mt-4 sm:gap-4 lg:grid-cols-3">
                    ...
                    <div className="space-y-3 sm:space-y-4 lg:col-span-2">
                        {/* About Card */}
                        <div className="rounded-xl bg-white p-3 shadow-sm transition-all duration-300 hover:shadow-md sm:rounded-xl sm:p-4">
                            <h2 className="text-base font-semibold text-gray-900 sm:text-lg">About</h2>
                            <div className="relative">
                                <div
                                    className="mt-2 overflow-hidden whitespace-pre-wrap text-sm leading-6 text-gray-600 transition-all duration-500 sm:mt-3"
                                    style={{ maxHeight: aboutExpanded ? "none" : ABOUT_INITIAL_HEIGHT }}
                                >
                                    {center.about}
                                </div>
                                {!aboutExpanded && hasMoreAbout && <div className="absolute bottom-0 left-0 right-0 h-12 bg-linear-to-t from-white to-transparent" />}
                            </div>
                            {hasMoreAbout && (
                                <button
                        ...
                        <div className="mx-auto w-100 rounded-xl bg-white p-3 shadow-sm transition-all duration-300 hover:shadow-md sm:rounded-xl sm:p-4 md:w-full">
                            ...
                            {campaigns.length > 0 ? (
                                <div className="group relative w-90 md:w-full">
                                    <div
                                        ref={scrollContainerRef}
                                        className="scrollbar-hide -mx-3 flex snap-x snap-mandatory gap-3 overflow-x-auto px-3 pb-3 sm:-mx-4 sm:gap-4 sm:px-4"
                                        style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
                                    >
                                        {campaigns.map((campaign) => (
                                            <div
                                                key={campaign.id}
                                                className="w-[280px] flex-none snap-start overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all duration-500 ease-out hover:-translate-y-1 hover:shadow-xl sm:w-[300px]"
                                            >
                                                ...
                                                <div className="relative h-40 overflow-hidden bg-gray-100 sm:h-44">
                                                    ...
                                                    <div className="absolute left-3 top-3">
                                                        <span className="inline-flex items-center rounded-full bg-white/90 px-3 py-1 text-[10px] font-semibold text-gray-700 shadow-sm backdrop-blur-sm sm:text-xs">
                                                            {campaign.category}
                                                        </span>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="p-4 sm:p-5">
                                                    <h3 className="mb-2 line-clamp-1 text-sm font-semibold leading-tight text-gray-900">{campaign.name}</h3>
                                                    <p className="mb-4 line-clamp-2 text-xs leading-relaxed text-gray-500">{campaign.details}</p>

                                                    <div className="mb-4 flex items-center justify-between border-b border-gray-50 pb-4">
                                                        <div>
                                                            <p className="text-[10px] font-medium uppercase tracking-wider text-gray-400">Raised</p>
                                                            <p className="text-base font-bold text-[#0a66c2] sm:text-lg">{formatNaira(campaign.raised || 0)}</p>
                                                        </div>
                                    ...
                                    {showLeftArrow && (
                                        <button
                                            onClick={() => scroll("left")}
                                            className="absolute left-0 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-gray-100 bg-white/95 shadow-lg backdrop-blur-sm transition-all duration-300 hover:scale-110 hover:bg-white hover:shadow-xl focus:outline-none"
                                    ...
                                    {showRightArrow && (
                                        <button
                                            ...
                                            className="absolute right-0 top-1/2 z-10 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-gray-100 bg-white/95 shadow-lg backdrop-blur-sm transition-all duration-300 hover:scale-110 hover:bg-white hover:shadow-xl focus:outline-none"
                                ...
                                <div className="py-6 text-center text-sm text-gray-500 sm:py-8">No active campaigns at the moment.</div>
                            )}
                        </div>
                    </div>

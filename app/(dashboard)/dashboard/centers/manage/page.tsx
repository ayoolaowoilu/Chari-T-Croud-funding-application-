...
export default function CenterRegistration() {
  ...
  useEffect(() => {
    const loadBanks = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch banks:', _err);
      } finally {
  ...
  const handleFileUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>, type: 'logo' | 'doc') => {
      ...
      setErrors((prev) => {
        const n = { ...prev };
        delete n[type === 'logo' ? 'logo' : 'documents'];
        return n;
      });
      try {
        ...
        setErrors((prev) => ({
          ...prev,
          [type === 'logo' ? 'logo' : 'documents']: _err.message || 'Upload failed. Try again.',
        }));
      } finally {
        setUploading(null);
        setCompressionProgress(0);
  ...
  const validate = (): boolean => {
    ...
    if (!verifiedAccountName)
      newErrors.bank_verification = 'Please verify your account before submitting';
    setErrors(newErrors);
    ...
    try {
      ...
      try {
        ...
        setErrors((prev) => ({
          ...prev,
          submit:
            'Failed to create payment sub-account. Please check your bank details and try again.',
        }));
        setSubmitting(false);
        return;
      }
      ...
      console.error('Registration submission _error:', _err);
      setErrors((prev) => ({
        ...prev,
        submit: _err.message || 'An unexpected error occurred. Please try again later.',
      }));
    } finally {
      setSubmitting(false);
    }
      ...
      {file.compression?.wasCompressed && (
        <>
          <span className="text-green-600">
            compressed to {formatFileSize(file.compression.compressedSize)}
          </span>
    ...
    return (
      <div className="w-screen h-screen bg-white">
        ...
        <main className="my-50 mx-auto w-full flex flex-col justify-center items-center ">
          ...
          <br />
          <p className="text-black font-bold my-6 text-center">
            Error Loading User Data Check Connection
          </p>
      ...
      <>
        ...
        <div className="w-full min-h-screen bg-slate-50">
          <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    You can register a maximum of 5 centers
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div className="w-24 h-2 bg-white rounded-full overflow-hidden border border-slate-200">
                    <div
                      ...
                      style={{ width: `${(centers.length / 5) * 100}%` }}
                    />
                  </div>
                </div>
              </div>
            ...
            {centers.length === 0 ? (
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {centers.map((item, index) => {
                  const _isVerified = item.is_verified_status?.toLowerCase() === 'verified';
                    ...
                    >
                      ...
                      <div className="p-5 pb-3">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            <div className="shrink-0 w-12 h-12 rounded-lg bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center">
                              ...
                              <svg
                                className={`w-5 h-5 text-slate-300 ${item.logourl ? 'hidden' : 'block'}`}
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                                strokeWidth={1.5}
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-xs text-slate-500 mt-0.5 truncate">
                                {item.registration_number || 'No reg. number'}
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-3">
                          ...
                          <div className="bg-slate-50 rounded-lg p-2.5 text-center">
                            <div className="text-lg font-bold text-slate-900">
                              ₦{formatAmount(Number(item.recived))}
                            </div>
                            <div className="text-[10px] font-medium text-slate-500 uppercase tracking-wide">
                              Received
                            </div>
            ...
            {centers.length >= 5 && (
              <p className="text-center mt-2 text-xs text-red-500">Maximum limit reached</p>
            )}
          </main>
        </div>
        <Footer />
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mb-6">
              <div className="p-6 sm:p-8">
                <div className="flex flex-col sm:flex-row sm:items-start gap-5">
                  ...
                  <div className="shrink-0 w-20 h-20 rounded-xl bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center">
                    ...
                    <svg
                      className={`w-8 h-8 text-slate-300 ${item.logourl ? 'hidden' : 'block'}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={1.5}
              ...
              <div className="grid grid-cols-3 border-t border-slate-100">
                <div className="p-4 text-center border-r border-slate-100">
                  ...
                  <div className="text-xs font-medium text-slate-500 uppercase tracking-wide mt-0.5">
                    Total Campaigns
                  </div>
                </div>
                <div className="p-4 text-center border-r border-slate-100">
            ...
            <div className="grid gap-6 md:grid-cols-2 mb-6">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6">
                ...
                <div className="relative">
                  <div className="text-sm text-slate-600 leading-relaxed overflow-y-auto max-h-40 pr-2 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-600">
                        {item.geo_location || 'Not provided'}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  setID(item.id);
                  setDocuments(
                    item.verification_documents?.map((d: any) => ({
                      cloudinary: {
                        url: d.url,
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden text-black">
              <div className="px-8 py-6 border-b border-gray-100">
                <h1 className="text-2xl font-semibold text-gray-900">
                  {isEdit ? 'Manage' : 'Register'} Charity Center
                </h1>
                <p className="mt-1 text-sm text-gray-500">
                  Complete the form below to {isEdit ? 'edit' : 'register'} your organization
                </p>
              </div>
                ...
                <section>
                  ...
                  <div className="space-y-5">
                    ...
                    <div>
                      ...
                      />
                      {errors.geo_location && (
                        <p className="mt-1 text-xs text-red-500">{errors.geo_location}</p>
                      )}
                    </div>
                  </div>
                </section>
                        ...
                        {uploading === 'logo' ? (
                          ...
                          <>
                            ...
                            <p className="text-xs text-gray-400">
                              JPG, PNG, WEBP, GIF (max 1MB, auto-compressed)
                            </p>
                          </>
                        )}
                      </div>
                    ...
                    {uploading === 'doc' ? (
                      ...
                      <>
                        ...
                        <p className="mt-2 text-sm text-gray-500">
                          Upload registration certificate, tax docs, etc.
                        </p>
                        <p className="text-xs text-gray-400">
                          Images only, max 1MB (auto-compressed)
                        </p>
                      </>
                    )}
                  </div>
                  {errors.documents && (
                    <p className="mb-3 text-xs text-red-500">{errors.documents}</p>
                  )}
                ...
                <section>
                  ...
                  <div className="space-y-4">
                    <div className="relative" ref={bankDropdownRef}>
                          ...
                          {filteredBanks.map((bank) => (
                            ...
                            >
                              ...
                              <div className="text-xs text-gray-500 mt-0.5">
                                Code: {bank.code} &middot; {bank.type}
                              </div>
                            </button>
                          ))}
                        </div>
                      ...
                      {errors.bank_bank_name && (
                        <p className="mt-1 text-xs text-red-500">{errors.bank_bank_name}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex justify-between mt-1">
                        <span className="text-xs text-gray-500">
                          {form.bank_details.account_number.length}/10 digits
                        </span>
                        {isVerifyingAccount && (
                      ...
                      {errors.bank_account_number && (
                        <p className="mt-1 text-xs text-red-500">{errors.bank_account_number}</p>
                      )}
                    </div>

                    {/* Manual verify button as fallback */}
                    {form.bank_details.bank_name &&
                      form.bank_details.account_number.length === 10 &&
                      !verifiedAccountName &&
                      !isVerifyingAccount && (
                        <button
                          type="button"
        ...
        <div className="w-screen h-full bg-white">
          <main className="my-50 flex flex-col justify-center items-center  bg-white ">
            ...
            <p className="text-gray-600  text-center font-bold">
              Your Center has been {isEdit ? 'Updated' : 'Added'} , Pending Verification.{' '}
            </p>
            <Button
              variant="secondary"
              onClick={() => redirect('/dashboard/donor?goto=charity')}
              size="lg"
              className="my-8"

...
const CATEGORIES: {
  value: CampaignPayload['category'];
  label: string;
  icon: string;
  description: string;
}[] = [
  {
    ...
    description: 'Medical aid & wellness programs',
  },
  {
    value: 'Education',
    label: 'Education',
  ...
  },
  {
    ...
    description: 'Local development & support',
  },
  {
    value: 'Business',
    label: 'Business',
    icon: 'M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z',
    description: 'Startups & economic growth',
  },
  {
    value: 'CroudFunding',
    label: 'Crowdfunding',
    icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
    description: 'General fundraising projects',
  },
];
...
export default function CampaignPage() {
  ...
  useEffect(() => {
    const previewUrl = upload.preview;
    return () => {
      if (previewUrl) URL.revokeObjectURL(previewUrl);
      if (abortControllerRef.current) abortControllerRef.current.abort();
    };
  }, [upload.preview, abortControllerRef]);
  ...
  const fetchAndAuth = useCallback(async () => {
    ...
    try {
      ...
      if (centerResp.is_verified_status !== 'verified') {
        setError(true);
        setErrorMsg(
          'Your center has not been verified yet. Please wait for verification before creating campaigns.',
        );
        setLoading(false);
        return;
      }
      ...
      if (resp.userData?.id !== centerResp.user_id) {
        setError(true);
        setErrorMsg(
          'You are not authorized to create campaigns for this center. Only the registered owner can manage campaigns.',
        );
        setLoading(false);
        return;
      }
  ...
  const handleFileSelect = useCallback(
    async (selectedFile: File) => {
      ...
      if (!validation.valid) {
        setUpload((prev) => ({
          ...prev,
          error: validation.error || 'Invalid file format. Please upload an image.',
        }));
        return;
      }
      ...
      try {
        const compressionResult = await compressImageIfNeeded(selectedFile, (progress) =>
          setUpload((prev) => ({ ...prev, progress: Math.round(progress * 0.3) })),
        );
  ...
  const handleSubmit = async () => {
    ...
    if (!imageUrl) {
      setUpload((prev) => ({
        ...prev,
        error: 'Please upload a campaign image or wait for upload to complete.',
      }));
      return;
    }
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="flex items-center justify-center min-h-[calc(100vh-200px)] px-4">
          <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center">
            ...
            <button
              onClick={() =>
                status === 'unauthenticated' ? redirect('/auth/signin') : fetchAndAuth()
              }
              className="w-full py-3 bg-[var(--brand)] text-white rounded-xl font-medium hover:bg-[var(--brand-hover)] transition-colors"
            >
                  ...
                  <div className="flex flex-col items-center">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold transition-all duration-300 ${
                        stage > s.id
                          ? 'bg-[var(--brand)] text-white shadow-lg shadow-teal-200'
                          : stage === s.id
                            ? 'bg-[var(--brand)] text-white shadow-lg shadow-emerald-200'
                    ...
                    <div className="mt-2 text-center hidden sm:block">
                      <p
                        className={`text-xs font-semibold ${stage >= s.id ? 'text-slate-900' : 'text-slate-400'}`}
                      >
                        {s.label}
                      </p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 space-y-8">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:ring-2 focus:ring-[var(--brand)] focus:border-transparent outline-none transition-all"
                maxLength={100}
              />
              <div className="flex justify-between mt-2">
                <p className="text-xs text-slate-500">
                  {nameCount < 4
                    ? `At least ${4 - nameCount} more character${4 - nameCount !== 1 ? 's' : ''}`
                    : 'Looks good'}
                </p>
            ...
            <div>
              ...
              <textarea
                ...
                rows={8}
                className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:ring-2 focus:ring-[var(--brand)] focus:border-transparent outline-none resize-none transition-all"
                maxLength={2000}
              />
              <div className="flex justify-between mt-2">
                <p
                  className={`text-xs ${detailsCount < 20 ? 'text-amber-600' : 'text-emerald-600'}`}
                >
                  {detailsCount < 20
            ...
            <div className="pt-4">
              <button
                ...
                className="w-full py-4 bg-[var(--brand)] text-white rounded-xl font-semibold hover:bg-[var(--brand-hover)] disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
              >
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 space-y-8">
                ...
                {CATEGORIES.map((cat) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div
                        className={`p-2 rounded-lg ${category === cat.value ? 'bg-[var(--brand)] text-white' : 'bg-slate-100 text-slate-600 group-hover:bg-slate-200'} transition-colors`}
                      >
                        <svg
                          className="w-5 h-5"
                          fill="none"
                      ...
                      <div>
                        <p
                          className={`font-semibold text-sm ${category === cat.value ? 'text-slate-900' : 'text-slate-700'}`}
                        >
                          {cat.label}
                        </p>
                    ...
                    {category === cat.value && (
                      <div className="absolute top-3 right-3">
                        <div className="w-5 h-5 bg-[var(--brand)] rounded-full flex items-center justify-center">
                          <svg
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:ring-2 focus:ring-[var(--brand)] focus:border-transparent outline-none transition-all"
              />
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button
                ...
                className="flex-1 py-4 bg-[var(--brand)] text-white rounded-xl font-semibold hover:bg-[var(--brand-hover)] disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
              >
                Review Campaign
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
              ...
              <p className="text-sm text-slate-500 mb-5">
                This image will be displayed as the main visual for your campaign. High-quality
                images increase donations by 3x.
              </p>
              ...
              {!upload.preview && !upload.url ? (
                <div
                  ...
                  className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all duration-200 ${
                    dragActive
                      ? 'border-[var(--brand)] bg-slate-50 scale-[1.02]'
                      : 'border-slate-300 hover:border-slate-400 hover:bg-slate-50'
                  }`}
                >
                  <div
                    className={`w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center transition-colors ${dragActive ? 'bg-[var(--brand)] text-white' : 'bg-slate-100 text-slate-400'}`}
                  >
              ...
              {upload.compressionInfo && upload.compressionInfo.wasCompressed && (
                <div className="mt-3 flex items-center gap-2 text-sm text-emerald-700 bg-emerald-50 px-4 py-3 rounded-lg border border-emerald-100">
                  ...
                  <span>
                    Optimized from {formatFileSize(upload.compressionInfo.originalSize)} to{' '}
                    {formatFileSize(upload.compressionInfo.compressedSize)}
                    <span className="font-semibold">
                      ({upload.compressionInfo.compressionRatio} saved)
                    </span>
                  </span>
                </div>
              )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div className="bg-slate-50 rounded-xl p-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[var(--brand)] text-white">
                        {category}
                      </span>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 sm:p-12 text-center space-y-8">
            <div className="relative">
              ...
              <div className="absolute inset-0 w-24 h-24 mx-auto">
                <div className="w-full h-full rounded-full border-2 border-[var(--brand)]/20 animate-ping opacity-20" />
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-slate-500 max-w-md mx-auto">
                Your campaign is now live and ready to receive donations. Share it with your
                community to maximize impact.
              </p>
            </div>
            ...
            <div className="max-w-lg mx-auto">
                ...
                <button
                  ...
                  className={`px-4 py-2.5 rounded-lg font-semibold text-sm transition-all flex items-center gap-2 shrink-0 ${
                    copied
                      ...
                      : 'bg-[var(--brand)] text-white hover:bg-[var(--brand-hover)] disabled:bg-slate-300'
                  }`}
                >
                  {copied ? (
                    <>
              ...
              <p className="text-xs text-slate-400 mt-3">
                Share this link with supporters to start receiving donations immediately
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto pt-4">
              <button
                ...
                className="flex-1 py-4 bg-[var(--brand)] text-white rounded-xl font-semibold hover:bg-[var(--brand-hover)] transition-all flex items-center justify-center gap-2"
              >

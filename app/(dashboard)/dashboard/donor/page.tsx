  ...
  return (
      ...
      className={`
        ...
        ${
          isOpen
            ...
            : 'bg-white/90 backdrop-blur-md shadow-[0_0_20px_rgba(15,118,110,0.18),0_0_40px_rgba(15,118,110,0.1),0_4px_15px_rgba(0,0,0,0.08)] hover:shadow-[0_0_30px_rgba(15,118,110,0.22),0_0_60px_rgba(15,118,110,0.14),0_6px_20px_rgba(0,0,0,0.1)] hover:scale-105 active:scale-95'
        }
      `}
      style={{
        top: '5.5rem',
    ...
    >
      <div className="relative">
        ...
        {!isOpen && (
          <>
            <span className="absolute inset-0 rounded-2xl bg-[var(--brand-soft)] animate-ping" />
            <span className="absolute -inset-1 rounded-2xl bg-[var(--brand-soft)]/80 animate-pulse" />
          </>
        )}
        {isOpen ? (
          <X className="w-5 h-5 text-gray-700 relative z-10" strokeWidth={2.5} />
        ) : (
          <Menu className="w-5 h-5 text-[var(--brand)] relative z-10" strokeWidth={2.5} />
        )}
      </div>
    </button>
  );
  ...
  return (
    <>
      ...
      >
        ...
        {/* Navigation */}
        <nav
          className={`flex-1 overflow-y-auto p-3 space-y-1 ${collapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
        >
          {NAV_ITEMS.map((item) => (
            <button
              key={item.id}
              ...
              `}
            >
              <span className={activeTab === item.id ? 'text-[var(--brand)]' : 'text-gray-400'}>
                {item.icon}
              </span>
              {!collapsed && (
                <>
                  ...
                  {item.badge && (
                    <span className="bg-[var(--brand-soft)] text-[var(--brand)] text-xs font-semibold px-2 py-0.5 rounded-full">
                      {item.badge}
                    </span>
                  )}
                </>
        ...
        <div className="shrink-0 p-3 border-t border-gray-100">
          {!collapsed ? (
            <div className="space-y-1">
              ...
              <div className="mt-3 pt-3 border-t border-gray-100">
                <div className="flex items-center gap-3 px-3">
                  <div className="w-8 h-8 rounded-full bg-linear-to-br from-[var(--brand)] to-[var(--brand-hover)] flex items-center justify-center text-white text-sm font-medium shrink-0 overflow-hidden">
                    {session?.user?.image ? (
                      <img
            ...
            <div className="flex justify-center">
              <div className="w-8 h-8 rounded-full bg-linear-to-br from-[var(--brand)] to-[var(--brand-hover)] flex items-center justify-center text-white text-sm font-medium overflow-hidden">
                {session?.user?.image ? (
                  <img
                    className="w-full h-full object-cover"
                    src={session.user.image}
        ...
        <div className="shrink-0 p-4 border-t border-gray-100 space-y-2">
          ...
          <div className="mt-3 pt-3 border-t border-gray-100">
            <div className="flex items-center gap-4 px-3">
              <div className="w-10 h-10 rounded-full bg-linear-to-br from-[var(--brand)] to-[var(--brand-hover)] flex items-center justify-center text-white font-medium shrink-0 overflow-hidden shadow-md">
                {session?.user?.image ? (
                  <img
                    className="w-full h-full object-cover"
                    src={session.user.image}

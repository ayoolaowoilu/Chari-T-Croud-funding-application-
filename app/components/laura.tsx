...
function findBestResponse(
  input: string,
): { response: string; steps?: string[]; links?: { label: string; href: string }[] } | null {
    ...
    setTimeout(
      () => {
        ...
        const botMsg: Message = {
          ...
          content:
            result?.response ||
            "I'm here to help! Try asking about starting a cause, donating, verification, or fees.",
          steps: result?.steps,
          links: result?.links,
          timestamp: new Date(),
        };
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
              {messages.map((msg) => (
                ...
                >
                  {/* Avatar */}
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${msg.type === 'bot' ? 'bg-blue-100' : 'bg-gray-200'}`}
                  >
                    {msg.type === 'bot' ? (
                  ...
                  {/* Content */}
                  <div
                    className={`max-w-[80%] space-y-2 ${msg.type === 'user' ? 'items-end' : 'items-start'}`}
                  >
                    <div
                      className={`relative p-3 rounded-2xl text-sm leading-relaxed ${msg.type === 'bot' ? 'bg-white border border-gray-200 text-gray-800 rounded-tl-sm' : 'bg-blue-600 text-white rounded-tr-sm'}`}
                    >
                      <p>{msg.content}</p>
                      {msg.type === 'bot' && (
                        <button
            ...
            <div className="p-3 bg-white border-t border-gray-200 shrink-0">
              ...
              <p className="text-[10px] text-gray-400 text-center mt-2">
                Laura is an AI assistant. For urgent issues,{' '}

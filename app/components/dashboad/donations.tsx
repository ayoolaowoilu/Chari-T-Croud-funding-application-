...
export default function DonationsPage() {
  ...
  }, [activeTab, email, fetchDonations, status]);
        ...
        <div className="flex border-b border-gray-200 bg-white">
          <button
            ...
            className={`flex-1 py-3.5 sm:py-4 text-xs sm:text-sm font-semibold transition-colors ${
              activeTab === 'gotten'
                ? 'text-[var(--brand)] border-b-2 border-[var(--brand)] bg-[var(--brand-soft)]/70'
        ...
        <div className="bg-white sm:mt-4 sm:mx-4 sm:rounded-xl sm:border sm:border-gray-200 overflow-hidden">
          {initialLoading ? (
            ...
            <div className="py-16 sm:py-20 text-center text-gray-400 px-4">
              <svg
                className="mx-auto h-10 w-10 sm:h-12 sm:w-12 mb-3"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                  ...
                  <div className="flex justify-between items-start gap-3 sm:gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5">
                        <span
                          className={`text-[10px] sm:text-xs px-2 py-0.5 rounded-full font-semibold shrink-0 ${
                            donation._to === 'center'
          ...
          {pagination && pagination.hasMore && !initialLoading && (
            <div className="p-4 border-t border-gray-100 text-center">
              <button
                onClick={loadMore}
                disabled={loading}
                className="w-full sm:w-auto px-6 py-2.5 text-xs sm:text-sm font-semibold text-[var(--brand)] bg-[var(--brand-soft)] rounded-lg hover:bg-[var(--brand-soft)]/80 transition-colors disabled:opacity-50"
              >
                {loading ? (

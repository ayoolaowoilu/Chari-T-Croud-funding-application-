...
function CampaignDetailsPanel({ campaign, onClose }: { campaign: Don; onClose: () => void }) {
  ...
  const pagedDonors = sortedDonors.slice(
    donorPage * DONOR_PAGE_SIZE,
    donorPage * DONOR_PAGE_SIZE + DONOR_PAGE_SIZE,
  );
  ...
  const tabs: { key: DetailTab; label: string; icon: typeof Target; count?: number }[] = [
    ...
    { key: 'donors', label: 'Donors', icon: Users, count: sortedDonors.length },
    {
      ...
      label: 'Subscribers',
      icon: Bell,
      count: subscribersLoaded ? subscribersTotal : (campaign.subscribed_count ?? undefined),
    },
    {
      key: 'comments',
      label: 'Comments',
      icon: MessageCircle,
      count: commentsLoaded ? commentsTotal : (campaign.comments_count ?? undefined),
    },
  ];
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-gray-900">
                    {new Date(Number(campaign.date_to_completion)).toLocaleDateString('en-US', {
                      month: 'short',
                      day: 'numeric',
                      year: 'numeric',
                    })}
...
export default function YourCam() {
  ...
  const fetchData = useCallback(
    async (isLoadMore = false, currentCursor?: string | number) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 pt-2">
                      ...
                      {/* Actions */}
                      <div
                        className="flex items-center gap-1.5 sm:flex-col"

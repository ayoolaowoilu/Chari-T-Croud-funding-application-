...
type userData = Pick<
  UserData,
  'created_at' | 'email' | 'full_name' | 'image' | 'is_verified' | 'bank_details'
> & {
  donations: number;
  recieved: number;
};
...
function ReceivedDonationsChart({ data }: { data: Donation[] }) {
  ...
  const centerAmount = data
    .filter((d) => d._to === 'center')
    .reduce((sum, d) => sum + d.ammount, 0);
  const normalAmount = data
    .filter((d) => d._to === 'normal')
    .reduce((sum, d) => sum + d.ammount, 0);
      ...
      <div className="mt-4 pt-4 border-t border-gray-50">
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <div className="bg-violet-50 rounded-xl p-3">
            <p className="text-[10px] text-violet-600 uppercase tracking-wider font-medium">
              Centers
            </p>
      ...
      <div className="mt-4 pt-4 border-t border-gray-50">
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="bg-[#0a66c2]/5 rounded-xl p-3">
            <p className="text-[10px] text-[#0a66c2] uppercase tracking-wider font-medium">
              Total Given
            </p>
      ...
      <ResponsiveContainer width="100%" height={280}>
        <ComposedChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
          ...
          <Bar
            ...
            name="received"
          />
          <Bar dataKey="given" fill="#0a66c2" radius={[6, 6, 0, 0]} maxBarSize={32} name="given" />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  );
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-gray-500">
                ...
                <span className="flex items-center gap-1">
                  <FileCheck className="w-3.5 h-3.5 text-[var(--brand)]" />
                  Verified payouts
                </span>
              </div>
            </div>
  ...
  const fetchDonations = useCallback(async () => {
    ...
    try {
      const [receivedRes, givenRes] = await Promise.all([
        fetch(
          `/api/dashboard/donations?email=${encodeURIComponent(session.user.email)}&type=gotten&page=1&limit=200`,
        ),
        fetch(
          `/api/dashboard/donations?email=${encodeURIComponent(session.user.email)}&type=given&page=1&limit=200`,
        ),
      ]);
      ...
      <div className="w-full min-h-0 p-4 sm:p-6 overflow-y-auto">
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[var(--brand)] text-sm font-medium cursor-pointer hover:text-[var(--brand-hover)] hover:underline">
              View more
            </span>
          </div>
          <LoadingCards mm />
      ...
      <div className="w-full min-h-full flex items-center justify-center p-4 sm:p-6">
        <div className="text-center max-w-sm">
          ...
          <button
            ...
            className="px-4 py-2 bg-[var(--brand)] text-white rounded-lg text-sm font-medium hover:bg-[var(--brand-hover)] transition-colors"
          >
            Try Again
          </button>
        </div>
    ...
    <div className="w-full min-h-0 space-y-6 sm:space-y-8 overflow-y-auto">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl font-semibold text-sm bg-[var(--brand)] hover:bg-[var(--brand-hover)] active:bg-[var(--brand-hover)] active:scale-95 text-white transition-colors duration-150 shrink-0"
        >
      ...
      <div>
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          ...
          <a
            ...
            className="text-[var(--brand)] text-sm font-medium hover:text-[var(--brand-hover)] hover:underline shrink-0 ml-2"
          >
            View more
          </a>
        </div>
